...
class TestBenchmarkRL2:
    ...
    def test_benchmark_rl2(self):
        ...
        df_g = pd.read_csv('data/local/experiment/progress.csv')
        df_g['Type'] = 'ProMP'
        data = df_g

        ax = sns.relplot(x='n_timesteps', y='train-AverageDiscountedReturn', hue='Type', kind='line', data=data)
        ax.axes.flatten()[0].set_title('HalfCheetah')

        plt.savefig('data/local/experiment/result_discounted_return.png')

        ax = sns.relplot(x='n_timesteps', y='train-AverageReturn', hue='Type', kind='line', data=data)
        ax.axes.flatten()[0].set_title('HalfCheetah')
