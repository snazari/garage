...
class HalfCheetahEnvMetaBase(HalfCheetahEnv_):
    ...
    def render(self, mode='human'):
        """Render the current state for viewing.

        Args:
            mode (str): Should be 'rgb_array' or 'human'. Defaults to 'human',
                which renders the current state in a viewer window. If set to
                'rgb_array', returns a (500, 500, 3) array of pixels.

        Returns:
            None or np.ndarray: See the mode argument.

        """
        # All new arguments are optional. Pylint really should understand this.
        # pylint: disable=arguments-differ,no-value-for-parameter
        # I don't understand why Pylint complains about this.
